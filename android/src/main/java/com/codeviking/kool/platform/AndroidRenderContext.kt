...
import com.codeviking.koolite.*
import com.codeviking.koolite.gl.*
...
class AndroidRenderContext(val context: Context, val glView: GLSurfaceView, val onKoolContextCreated: (AndroidRenderContext) -> Unit) :
    ...
    init {
        ...
        com.codeviking.koolite.util.Log.printer = { lvl, tag, message ->
            var t = tag ?: "Kool"
            if (t.length > 23) {
                t = t.substring(0..22)
            }
            when (lvl) {
                com.codeviking.koolite.util.Log.Level.TRACE -> Log.d(t, message)
                com.codeviking.koolite.util.Log.Level.DEBUG -> Log.d(t, message)
                com.codeviking.koolite.util.Log.Level.INFO -> Log.i(t, message)
                com.codeviking.koolite.util.Log.Level.WARN -> Log.w(t, message)
                com.codeviking.koolite.util.Log.Level.ERROR -> Log.e(t, message)
                else -> { }
            }
        }
