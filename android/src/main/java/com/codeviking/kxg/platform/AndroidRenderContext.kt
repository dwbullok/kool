...
/**
 * Android specific implementation of Kxg's RenderContext
 */
class AndroidRenderContext(val context: Context, val glView: GLSurfaceView, val onKxgContextCreated: (AndroidRenderContext) -> Unit) :
        KxgContext(), GLSurfaceView.Renderer, GLSurfaceView.EGLContextFactory, View.OnTouchListener {
    ...
    constructor(context: Context, onKxgContextCreated: (AndroidRenderContext) -> Unit) :
            this(context, GLSurfaceView(context), onKxgContextCreated)
    ...
    //
    // Functions overridden from KxgContext
    //
    ...
    override fun createContext(egl: EGL10, display: EGLDisplay, config: EGLConfig): EGLContext {
        ...
        if (egl.eglGetError() == EGL10.EGL_SUCCESS) {
            ...
            if (egl.eglGetError() != EGL10.EGL_SUCCESS) {
                destroyContext(egl, display, context)
                throw KxgException("Failed to create OpenGL ES context")
            }
        }
        return context
    }
    ...
    override fun onSurfaceCreated(gl: GL10, config: EGLConfig) {
        ...
        // don't call onKxgContextCreated() yet, we first wan't to know how big our viewport is
        //onKxgContextCreated(this)
    }
