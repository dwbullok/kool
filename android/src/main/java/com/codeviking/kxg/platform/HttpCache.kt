...
class HttpCache private constructor(val cacheDir: File) {
    ...
    private fun rebuildIndex() {
        synchronized(cache) {
            ...
            if (!cacheDir.exists() && !cacheDir.mkdirs()) {
                throw KxgException("Failed to create cache directory")
            }
        }
    ...
    companion object {
        ...
        suspend fun loadHttpResource(url: String): File {
            val inst = instance ?: throw KxgException("Default cache used before initCache() was called")
            return inst.loadHttpResource(url) ?: throw KxgException("Failed loading http resource $url")
        }
    }
