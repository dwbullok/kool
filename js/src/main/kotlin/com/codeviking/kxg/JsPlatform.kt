...
fun createContext(props: JsContext.InitProps): KxgContext = JsImpl.createContext(props)
...
internal object JsImpl {
    ...
    val gl: WebGLRenderingContext
        get() = ctx?.gl ?: throw KxgException("Platform.createContext() not called")
    ...
    fun createContext(props: JsContext.InitProps): KxgContext {
        if (ctx != null) {
            throw KxgException("Context was already created (multi-context is currently not supported in js")
        }
        ctx = JsContext(props)
        return ctx!!
    }
