...
class TextureManager internal constructor() : SharedResManager<TextureProps, TextureResource>() {
    ...
    fun unbindTexture(texture: Texture?, ctx: KxgContext) {
        texture ?: return

        if (texture.isValid) {
            val texRes = texture.res ?: throw KxgException("TextureResource is null although it was created")
            if (texRes.texUnit >= 0) {
    ...
    fun bindTexture(texture: Texture, ctx: KxgContext, makeActive: Boolean = false): Int {
        if (!texture.isValid) {
            nextTexUnit()
    ...
    internal fun createTexture(props: TextureProps, ctx: KxgContext): TextureResource {
        return addReference(props, ctx)
    }

    internal fun deleteTexture(texture: Texture, ctx: KxgContext) {
        val res = texture.res
        if (res != null) {
    ...
    private fun loadTexture(texture: Texture, ctx: KxgContext) {
        val res = texture.res ?: throw KxgException("Can't load a texture that wasn't created")
    ...
    override fun createResource(key: TextureProps, ctx: KxgContext): TextureResource {
        val texRes = TextureResource.create(key.target, key, ctx)
        bindToActiveTexUnit(texRes)
        return texRes
    }

    override fun deleteResource(key: TextureProps, res: TextureResource, ctx: KxgContext) {
        res.delete(ctx)
    }
}
