...
abstract class Camera(name: String = "camera") : Node(name) {
    ...
    open fun updateCamera(ctx: KxgContext) {
    ...
    fun computePickRay(pickRay: Ray, ptr: InputManager.Pointer, ctx: KxgContext): Boolean {
        return ptr.isValid && computePickRay(pickRay, ptr.x, ptr.y, ctx)
    }

    fun computePickRay(pickRay: Ray, screenX: Float, screenY: Float, ctx: KxgContext): Boolean {
    ...
    fun initRayTes(rayTest: RayTest, ptr: InputManager.Pointer, ctx: KxgContext): Boolean {
        return ptr.isValid && initRayTes(rayTest, ptr.x, ptr.y, ctx)
    }

    fun initRayTes(rayTest: RayTest, screenX: Float, screenY: Float, ctx: KxgContext): Boolean {
        rayTest.clear()
    ...
    fun projectScreen(world: Vec3f, ctx: KxgContext, result: MutableVec3f): Boolean {
    ...
    fun unProjectScreen(screen: Vec3f, ctx: KxgContext, result: MutableVec3f): Boolean {
...
open class OrthographicCamera(name: String = "orthographicCam") : Camera(name) {
    ...
    override fun updateCamera(ctx: KxgContext) {
        if (isClipToViewport) {
            left = 0f
