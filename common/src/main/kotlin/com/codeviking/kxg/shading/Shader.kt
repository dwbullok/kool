...
abstract class Shader : GlObject<ProgramResource>() {
    ...
    abstract fun generate(ctx: KxgContext)
     ...
     */
    fun isBound(ctx: KxgContext): Boolean {
     ...
     */
    open fun onLoad(ctx: KxgContext) {
        generate(ctx)
     ...
     * @param ctx    the graphics engine context
     */
    abstract fun onBind(ctx: KxgContext)
     ...
     * @param ctx    the graphics engine context
     */
    abstract fun onMatrixUpdate(ctx: KxgContext)
     ...
     */
    protected open fun findAttributeLocation(attribName: String, ctx: KxgContext): Int {
     ...
     */
    private fun enableAttribute(attribute: Attribute, ctx: KxgContext): Boolean {
     ...
     */
    private fun enableAttribute(attribute: Attribute, location: Int, ctx: KxgContext) {
        if (location >= 0) {
     ...
     */
    protected open fun findUniformLocation(uniformName: String, ctx: KxgContext): Any? {
        val ref: ProgramResource? = res
        if (ref != null) {
     ...
     * @param ctx    the graphics engine context
     */
    open fun bindMesh(mesh: Mesh, ctx: KxgContext) {
        for (i in attributeLocations.indices) {
            val attrib = attributeLocations[i]
            val binder = mesh.meshData.attributeBinders[attrib.descr] ?:
                    throw KxgException("Mesh must supply an attribute binder for attribute ${attrib.descr.name}")
     ...
     * @param ctx    the graphics engine context
     */
    open fun unbindMesh(ctx: KxgContext) {
        for (i in attributeLocations.indices) {
