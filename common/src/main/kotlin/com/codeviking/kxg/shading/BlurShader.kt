...
class BlurredBackgroundHelper(private val texSize: Int = 256,
    ...
    fun updateDistortionTexture(node: Node, ctx: KxgContext, bounds: BoundingBox = node.bounds) {
    ...
    private fun doBlurring(ctx: KxgContext) {
    ...
    private fun renderFb(fb: Framebuffer, mesh: Mesh, shader: Shader, ctx: KxgContext) {
        fb.bind(ctx)
    ...
    private fun addToTexBounds(cam: Camera, node: Node, x: Float, y: Float, z: Float, ctx: KxgContext) {
        tmpVec.set(x, y, z)
