...
data class MeshData(
    ...
    init {
        numVertices = (attributes[ATTRIB_POSITIONS] ?: throw KxgException("MeshData does not contain positions")).size / 3
        ...
        for ((name, attrib) in attributes) {
            if (attrib.size / attrib.type.size != numVertices) {
                throw KxgException("Mesh attribute $name has wrong value count: ${attrib.size} (should be ${numVertices * attrib.type.size}, type: ${attrib.type})")
            }
        }
    }
