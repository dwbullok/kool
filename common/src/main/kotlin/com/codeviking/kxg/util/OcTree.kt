...
class OcTree<T: Any>(itemAdapter: ItemAdapter<T>, items: List<T> = emptyList(),
    ...
    init {
        ...
        if (bounds.isEmpty) {
            throw KxgException("OcTree bounds are empty, specify bounds manually")
        }
    ...
    inner class OcNode(val nodeBounds: BoundingBox, depth: Int, val bucketSz: Int) : Node(depth) {
        ...
        init {
            if (depth > MAX_DEPTH) {
                throw KxgException("Octree is too deep")
            }
            if (!accurateBounds) {
        ...
        internal fun clear() {
            if (depth != 0) {
                throw KxgException("clear() is only allowed for root node")
            }
            mutItems.clear()
            children.clear()
            size = 0
